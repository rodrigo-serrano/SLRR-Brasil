...
// OSD stuff:
	...
	// Main Osd Group:
		...
		m.addItem( "Chevrolet Astra", CMD_AMBULANCE );
		m.addItem( "Fiat Fiorino Sedex", CMD_ARMOREDVAN );
		m.addItem( "Volkswagen Gol", CMD_BADGE );
		m.addItem( "Volkswagen Santana", CMD_CIVILVAN );
		m.addItem( "Onibus Rodoviario", CMD_COACH );
		m.addItem( "Alfa Romeo 164", CMD_ERBILAC );
		m.addItem( "Caminhao de Bombeiros", CMD_FIREENGINE );
		m.addItem( "Chevrolet Opala", CMD_NINJA );

		m = osd.createMenu( Menu_Right, 0.98, -0.45, 0 );
		m.addItem( "Viatura Policial", CMD_POLICE );
		m.addItem( "Mercedes Benz 1113", CMD_SCHOOLBUS );
		m.addItem( "Chevrolet Kadett", CMD_STALLION );
		m.addItem( "Fiat Premio", CMD_SUNSET );		
		m.addItem( "Chevrolet Diplomata Taxi", CMD_TAXI );
		m.addItem( "Volkswagen Fusca", CMD_TEG );
		m.addItem( "Chevrolet Vectra", CMD_WAGON );
